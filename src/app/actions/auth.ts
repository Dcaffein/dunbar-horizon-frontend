...
export async function sendVerificationEmail(email: string) {
  ...
  try {
    ...
    await apiClient.post("/api/auth/verifications", {
      email,
      redirectPage,
    });
...
export async function signupAction(
  ...
  try {
    await apiClient.post<void, SignUpRequest>("/api/auth/users", {
      email,
      nickname,
      password,
    });
...
export async function verifyEmailAction(token: string) {
  ...
  try {
    await apiClient.patch("/api/auth/verifications", undefined, {
      params: { token },
    });
