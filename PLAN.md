...
바이트가 아니라 **글자 수 255 경계**다. 확실한 것은 **Bean Validation 이 없다는 것**뿐이다 —
있었으면 400 + `validation` 이 왔을 것이다. 원인이 DB 컬럼 길이인지는 응답만으로 알 수 없다(추정).
...
판정은 문구에 **한글이 하나도 없으면** Bean Validation 기본 메시지로 간주한다.
