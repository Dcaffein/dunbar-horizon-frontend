...
## 2. 레이블링 및 관리 관련 (Labeling)

| 개념 (Korean)    | 변수명/타입 (English)   | 설명 (Description)                               |
| :--------------- | :---------------------- | :----------------------------------------------- |
| 레이블           | `Label`                 | 친구들을 그룹화하기 위한 분류 도메인             |
| 레이블 이름      | `labelName`             | 사용자가 정의한 레이블 명칭 (최대 20자)          |
| 노출 여부        | `exposure`              | 타인에게 해당 레이블 정보를 공개할지 여부        |
| 레이블 멤버      | `memberId`              | 특정 레이블에 포함된 친구의 ID (`friendId`와 매핑) |
| 레이블 생성 요청 | `LabelCreateRequest`    | 레이블 생성을 위한 DTO 명칭                      |
| 멤버 추가 요청   | `LabelMemberAddRequest` | 레이블에 친구를 추가하기 위한 DTO 명칭           |
