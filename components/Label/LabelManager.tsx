...
export default function LabelManager({ selectedNodeId, friends }: LabelManagerProps) {
  ...
  function handleAddMember() {
    if (!selectedLabelId || !selectedNodeId) return;

    const memberId = Number(selectedNodeId);
    addMember(selectedLabelId, memberId);
    setSuccessMessage(`${selectedFriendDisplayName}을(를) 라벨에 추가했습니다.`);
    setTimeout(() => setSuccessMessage(null), 2000);
  }
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        ...
        {successMessage && (
          <p className="text-xs text-indigo-600 text-center mt-2">{successMessage}</p>
        )}
      </div>
      ...
      <div>
        ...
        <div className="flex flex-col gap-2">
          {labels.map((label) => (
            ...
            >
              <span className="text-sm font-medium truncate">{label.labelName}</span>
              <div className="flex items-center gap-2 shrink-0 ml-2">
                <span className="text-xs text-gray-400">
                  {label.memberIds.length}명
                </span>
                <span
      ...
      {/* 멤버 추가 섹션 */}
      <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        <p className="text-xs font-bold text-gray-500 mb-3">선택한 친구를 라벨에 추가</p>
